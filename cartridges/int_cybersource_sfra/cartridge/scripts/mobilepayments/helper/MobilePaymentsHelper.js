...
function validateMobilePaymentRequest(order) {
    ...
    if (!empty(order.getPaymentInstruments(CybersourceConstants.METHOD_ApplePay))) {
    ...
    } if (jsonObj.cryptogram.length > 40) {
    ...
    return {
        success: true, RequestType: 'INAPP', OrderNo: jsonObj.orderID, requestParam: requestParam, order: order, MobilePaymentType: paymentMethod
    };
}
...
function PrepareAuthRequestObjects(lineItemCtnr) {
    ...
    return {
        success: true, billTo: billTo, shipTo: shipTo, purchaseObject: purchaseObject, items: items
    };
}
